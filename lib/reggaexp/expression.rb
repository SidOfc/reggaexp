...
module Reggaexp
  ...
  class Expression < Engine
    ...
    def start_of_line(*args, **opts, &block)
      parse(*args, **opts.merge(prepend: '^'), &block)
    end

    def start_of_string(*args, **opts, &block)
      parse(*args, **opts.merge(prepend: '\A'), &block)
    end

    def end_of_line(*args, **opts, &block)
      parse(*args, **opts.merge(append: '$'), &block)
    end

    def end_of_string(*args, **opts, &block)
      parse(*args, **opts.merge(append: '\z'), &block)
    end

    def one_or_more(*args, **opts, &block)
      parse(*args, **opts.merge(quantifier: '+'), &block)
    end
    ...
    def between(rng_or_ary, *args, **opts, &block)
      parse(*args, **opts.merge(quantifier: [rng_or_ary.first, rng_or_ary.last]), &block)
    end

    def at_least(amount, *args, **opts, &block)
      parse(*args, **opts.merge(quantifier: [amount, nil]), &block)
    end
    alias min at_least

    def at_most(amount, *args, **opts, &block)
      parse(*args, **opts.merge(quantifier: [nil, amount]), &block)
    end
    alias max at_most

    def group(*args, **opts, &block)
      non_capture = true unless opts.key?(:as) || opts[:capture]
      parse(*args, **opts.merge(non_capture: non_capture), &block)
    end
    ...
    def not(*args, **opts, &block)
      parse(*args, **opts.merge(non_capture: false, capture: false, as: nil,
    ...
    def or(*args, **opts, &block)
      opts.delete :or
      parse or: true
      parse(*args, **opts, &block) if args.any? || block_given?
